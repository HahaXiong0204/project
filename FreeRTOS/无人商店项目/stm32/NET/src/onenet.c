...
void Send_data_kc(void)
{
    ...
    if(send_cjson != NULL)
    {
		...
		cJSON_AddNumberToObject(send_cjson,"ID_Cart",ID_Cart);
		cJSON_AddNumberToObject(send_cjson,"Water_num",Water_num+Water_g);
		cJSON_AddNumberToObject(send_cjson,"Cookie_num",bingan_num+bingan_g);
		cJSON_AddNumberToObject(send_cjson,"Juice_num",yinlian_num+yinliao_g);
...
void Send_data_uing(void)
{
    ...
    if(send_cjson != NULL)
    {
		cJSON_AddStringToObject(send_cjson,"Device_ID",PROID);
		cJSON_AddNumberToObject(send_cjson,"ID_Cart",ID_Cart);
		cJSON_AddNumberToObject(send_cjson,"Water",Water_g);
		cJSON_AddNumberToObject(send_cjson,"Cookie",bingan_g);
		cJSON_AddNumberToObject(send_cjson,"Juice",yinliao_g);
		
		cJSON_AddNumberToObject(send_cjson,"Water_num",Water_num);
		cJSON_AddNumberToObject(send_cjson,"Cookie_num",bingan_num);
		cJSON_AddNumberToObject(send_cjson,"Juice_num",yinlian_num);
		
		cJSON_AddNumberToObject(send_cjson,"all_num",calu_num);
...
//					OneNet_Send_Int("ack",1);
	...
	// ===========================================解析接收到的数据=====================================================
					...
					if(cJSON_GetObjectItem(cjson,"kc")!=NULL)
					{
						nua = cJSON_GetObjectItem(cjson,"kc")->valueint;
						printf("csjon解析成功 nua = %d\r\n",nua);
					}
					...
					if(cJSON_GetObjectItem(cjson,"Water_num")!=NULL)
					{
						Water_num = cJSON_GetObjectItem(cjson,"Water_num")->valueint;
						printf("csjon解析成功 Water_num = %d\r\n",Water_num);
					}
					
					if(cJSON_GetObjectItem(cjson,"Cookie_num")!=NULL)
					{
						bingan_num = cJSON_GetObjectItem(cjson,"Cookie_num")->valueint;
						printf("csjon解析成功 Cookie_num = %d\r\n",bingan_num);
					}
					if(cJSON_GetObjectItem(cjson,"Juice_num")!=NULL)
					{
						yinlian_num = cJSON_GetObjectItem(cjson,"Juice_num")->valueint;
						printf("csjon解析成功 Juice_num = %d\r\n",yinlian_num);
					}
