...
//    show_picture(100,100,320,320,(uint16_t *)gImage_pic);
...
void Send_Zigbee(void)
{
	...
	sprintf(zigbee_buff,"tem:%.f  hum:%.2f",temperature,humidity);
...
void Send_tianwen(uint8_t mode)
{
	...
	switch(mode)
	{
		...
		case 11:{    /*总消费为*/
//			x_buff[0] = 0x0b;
//			x_buff[1] = (int)Total_amount;
//			x_buff[2] = ((int)Total_amount*100)%100;
			break;
		}	
		case 12:{    /*支付成功*/
			x_buff[0] = 0x0c;
...
{
	send_buffer = msg;
		// xQueueSendToFrontFromISR 放前面      xQueueSendToBackFromISR 放后面
	if( xQueueSendToFrontFromISR( hsend_queue, &send_buffer, 0 ) != pdPASS )
	{
