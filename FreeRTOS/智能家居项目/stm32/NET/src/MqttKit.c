...
char Submit_Topc[64];
const int8 Publish_Buf[2][16] = {"one"}; // 发布的主题
...
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
    ...
    if(MQTT_PacketPublish(MQTT_PUBLISH_ID, Publish_Topc, NULL, send_len, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
    {
//        printf("chuli = %s,chuli_len=%d\r\n", mqttPacket->_data, mqttPacket->_len);
