...
uint16_t OneNet_FillBuf(char *buf, uint8_t again_flag)
{
    ...
    if(send_cjson != NULL)
    {
        ...
        if(again_flag)
            cJSON_AddNumberToObject(send_cjson, "Break_flag", 1); // 重发标志位
		
		cJSON_AddNumberToObject(send_cjson, "Device_ID", DERVICE_ID);	    	//添加整型数字
        cJSON_AddNumberToObject(send_cjson, "age", id++);	    	//添加整型数字
		cJSON_AddNumberToObject(send_cjson, "ID_Cart", ID_Cart);			//添加状态值
        cJSON_AddNumberToObject(send_cjson, "Temperature_clod", temperature);	//添加浮点型数字
		cJSON_AddNumberToObject(send_cjson, "Temperature_hot", Tc);	//添加浮点型数字
        cJSON_AddNumberToObject(send_cjson, "TDS1", Turbidity);	//添加浮点型数字
        cJSON_AddNumberToObject(send_cjson, "TDS2", voltage);	//添加浮点型数字
...
void OneNet_Send_user_End(void)
{
    ...
    if(send_cjson != NULL)
    {
		...
		*/
        ...
        cJSON_AddNumberToObject(send_cjson, "year_start", year_time_start);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "moon_start", moon_time_start);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "day_start", sun_time_start);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "hour_start", hour_time_start);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "minute_start", minute_time_start);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "second_start", second_time_start);			//添加状态值
		
		
		
        cJSON_AddNumberToObject(send_cjson, "year", year_time);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "moon", moon_time);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "day", sun_time);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "hour", hour_time);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "minute", minute_time);			//添加状态值
		cJSON_AddNumberToObject(send_cjson, "second", second_time);			//添加状态值
            ...
            else
            {
                ...
                */
                if(cJSON_GetObjectItem(cjson, "time") != NULL)
                {
					...
					OneNet_Send_One_Int_Data("ID_Cart",ID_Cart);    // 发送卡号
