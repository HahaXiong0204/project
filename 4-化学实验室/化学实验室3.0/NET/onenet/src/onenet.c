...
extern u8 th_temp,th_humidity,th_fire,th_MQ135,th_MQ2,th_Lighting;
extern u8 mode;
...
unsigned char OneNet_FillBuf(char *buf)
{
    ...
    memset(text, 0, sizeof(text));
	sprintf(text, "TH_Lighting:%d,", th_Lighting);
	strcat(buf, text);
...
//						printf("json pack into cjson error...");
                ...
                value = 0;  
                value = atoi(cJSON_GetObjectItem(cjson,"TH_Lighting")->valuestring);
                if(value!=0)
                {
                    th_Lighting = value;
                }
