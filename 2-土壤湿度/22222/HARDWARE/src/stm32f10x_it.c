	...
	{  
		...
		{                                     
			...
			{                                     			 //处于指令配置状态时，非零值才保存到缓冲区	
				...
				USART1->DR = Usart2_RxBuff[Usart2_RxCounter]; 
...
{		
	...
	{ 
		...
		sprintf(temp, "{\"temperature\":\"%d\"}",temperature );//构建报文,“temperature”为OneNET平台的数据流名称
		head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		...
		memset(tempAll, 0, 100);				//清空缓冲区2
		memset(head1,   0, 3);					//清空MQTT头	
		sprintf(temp, "{\"humidity\":\"%d\"}", humidity);//构建报文,“humidity”为OneNET平台的数据流名称
        head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		head1[2] = strlen(temp);  				//剩余长度	
		sprintf(tempAll, "%c%c%c%s", head1[0], head1[1], head1[2], temp);
		...
		memset(tempAll, 0, 100);				//清空缓冲区2
		memset(head1,   0, 3);					//清空MQTT头	
		sprintf(temp, "{\"fan\":\"%s\"}", (char*)hum);//构建报文,“humidity”为OneNET平台的数据流名称
        head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		head1[2] = strlen(temp);  				//剩余长度	
		sprintf(tempAll, "%c%c%c%s", head1[0], head1[1], head1[2], temp);
		...
		memset(tempAll, 0, 100);				//清空缓冲区2
		memset(head1,   0, 3);					//清空MQTT头	
		sprintf(temp, "{\"shui\":\"%s\"}", (char*)hum);//构建报文,“humidity”为OneNET平台的数据流名称
        head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		head1[2] = strlen(temp);  				//剩余长度	
		sprintf(tempAll, "%c%c%c%s", head1[0], head1[1], head1[2], temp);
		...
		memset(tempAll, 0, 100);				//清空缓冲区2
		memset(head1,   0, 3);					//清空MQTT头	
		sprintf(temp, "{\"Lighting\":\"%s\"}",(char*)Light);//构建报文,“Lighting”为OneNET平台的数据流名称
        head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		head1[2] = strlen(temp);  				//剩余长度	
		sprintf(tempAll, "%c%c%c%s", head1[0], head1[1], head1[2], temp);
		...
		memset(tempAll, 0, 100);				//清空缓冲区2
		memset(head1,   0, 3);					//清空MQTT头	
		sprintf(temp, "{\"airquality\":\"%s\"}",(char*)Ga);//构建报文,“Lighting”为OneNET平台的数据流名称
		head1[0] = 0x03; 						//固定报头
		head1[1] = 0x00; 						//固定报头
		head1[2] = strlen(temp);  				//剩余长度	
		sprintf(tempAll, "%c%c%c%s", head1[0], head1[1], head1[2], temp);
