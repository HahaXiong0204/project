...
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, "one", NULL, send_len + 3, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
	{
...
uint1 MQTT_PacketSaveBinData(const int8 *name, int16 file_len, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, "one", payload, payload_size, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
		result = 0;
//	if(MQTT_PacketSubscribe(MQTT_PUBLISH_ID,MQTT_QOS_LEVEL1, "$creq", send_len + 3, mqttPacket) == 0){}
