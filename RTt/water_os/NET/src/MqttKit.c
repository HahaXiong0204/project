...
char Publish_Topc[64];
char Publish_Topc_ID[64];
char Submit_Topc[64];
char Submit_Topc_ID[64];
const char *Publish_Buf[] = {Publish_Topc,Publish_Topc_ID};  // 发布的主题
const char *topics[] = {Submit_Topc};      // 订阅的主题
const char *topics_id[] = {Submit_Topc_ID};
//unsigned short choice = 0;	//发送间隔变量
//#define Publish_Topc Publish_Buf[choice]
...
void MQTT_DeleteBuffer(MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	{
		printf("mqtt =============%s ,,,szie = %d\r\n",mqttPacket->_data,data_mqtt_size);
		temp_data = mqttPacket->_data;
		for(data_mqtt_size = 0;data_mqtt_size<mqttPacket->_size;data_mqtt_size++)
			MQTT_FreeBuffer(temp_data++);
	}
...
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, Publish_Buf[0], NULL, send_len, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
	{
		printf("发布到 %s\r\n",Publish_Buf[0]);
	}
	else
...
//==============================================
uint1 MQTT_Own_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{

	if(MQTT_PacketPublish(MQTT_PUBLISH_ID+1, "one", NULL, send_len + 3, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
	{
