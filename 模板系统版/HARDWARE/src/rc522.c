...
unsigned char DATA1[16]= {0x12,0x34,0x56,0x78,0x90,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff};
...
int RC522_ReadCard(void)
{
    ...
    {
        printf("\r\n读卡成功\r\n");
		printf("RC522_ReadCard() run finished after 1 second!\r\n");
        delay_ms(1000);
    }
    return status;
}
...
void SPI1_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOB, ENABLE );//PORTA、B时钟使能
...
void ShowID(u8 *p)
{
    ...
    {
        RFID_num[i*2] = p[i] / 16;
        RFID_num[i*2] > 9 ? (RFID_num[i*2] += '7') : (RFID_num[i*2] += '0');
        RFID_num[i*2+1] = p[i] % 16;
        RFID_num[i*2+1] > 9 ? (RFID_num[i*2+1] += '7') : (RFID_num[i*2+1] += '0');
    }
    RFID_num[8] = 0;
